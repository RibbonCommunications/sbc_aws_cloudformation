...
Ribbon recommends m5.xlarge or higher instance type if this instance
type is available in your zone. Use c5.2xlarge instance type or higher
to handle more calls with transcoding.

As of release 7.2S405, only following Instance types are supported for
deployment in AWS:
...
Any prior release deployments using C3, C4 or M4 instance types will
need to be migrated to the newer instance types.
...
5.  Select a strategy for the placement group -
    ...
    1.  A cluster placement group clusters instances in a single
        Availability Zone to provide low latency and higher network
        throughput.
...
Each SBC deployment requires access to the internet for management and
potentially for signaling and media, depending on customer needs.
...
7.  Select the created internet gateway and the select Attach to VPC
...
An SBC deployment requires a VPC with sufficient IP space to support 4
or 5 IPv4 subnets:
...
4.  Click on "Create subnet" to create a subnet for MGT0 using a CIDR
    block, e.g. x.x.0.0/20

5.  Click on "Create subnet" to create a subnet for HA0 using an IPv4
    CIDR block, e.g. x.x.16.0/20

6.  Click on "Create subnet" to create a subnet for PKT0 usingan IPv4
    CIDR block, e.g. x.x.32.0/20

7.  Click on "Create subnet" to create a subnet for PKT1 usingan IPv4
    CIDR block, e.g. x.x.48.0/20

8.  IF a High-Availability Forwarded Engine instance is to be deployed
    with the SBC, a public-facting subnet must be used. An existing
    public-facing subnet can be re-used. If not available, click on
    "Create subnet" to create a subnet for HFE Public-facing usingan
...
destined for a target within the VPC (10.0.0.0/16) is covered by the
...
16. You will be returned to the route table screen. The Explicitly
    ...
    associated with your route table.

# Creating Security Groups for SBC

Security groups must be created to support Management (MGT0), High
Availability (HA0), PKT0 and PKT1 subnets for the SBC.

Before creating the security groups, review the recommended security
group rule settings in the following section.

## Security Group Rules Overview

## Inbound Security Group Rules

It is recommended to open the following ports using Inbound/Ingress
rules in the security groups associated with management, HA and packet
subnets.
...
The source ranges for the packet security group may be external IP
address ranges or they may be the HFE private subnet CIDR if a
High-availability Forwarding Engine is present in the configuration.
...
If specific ports are opened in outbound security group rules, the
...
Considering that the SIP signaling port in SBC configuration is set to
the default port (5060), the port numbers for UDP/TCP are set to 5060
and 5061.
...
The SBC requires 2 key pairs to be supplied at deployment time to
...
13. In the search window next to **Filter policies**, type the name of
    the policy you just created, then select it from the list.
...
8.  Put a checkmark next to Keep all versions of an object in the same
    bucket.
...
13. Click **Create bucket**.  
    ...
    returned to the S3 buckets page.
...
3.  Locate the Amazon Linux 2 AMI (HVM), SSD Volume Type image in the
