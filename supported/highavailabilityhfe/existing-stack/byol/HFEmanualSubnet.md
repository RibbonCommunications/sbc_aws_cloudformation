...
This solution uses a CloudFormation Template to launch a pair of Ribbon SBC VEs fronted by a forwarding engine VE in an Amazon Virtual Private Cloud, using BYOL (bring your own license) licensing. **In this deployment a  subnet is must be manually created prior to the deployment for use as a private subnet between the HFE and SBC.**
